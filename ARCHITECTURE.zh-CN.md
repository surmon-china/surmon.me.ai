...
- `GET /admin/chat-sessions/:id` → 查询指定会话的完整消息列表（ChatMessages）

为了使 AI 服务中的鉴权业务简洁易维护，admin 鉴权直接转发 Authorization 头至 NodePress `/admin/verify-token` 进行验证，不在本服务存储管理员凭证和验证逻辑。
...
使用 HMAC-SHA256 签发 Token，`sessionId` 作为 payload。**[代码](src/chat-user/signature/index.ts)**
...
可以在 [`CONFIG.CHAT_AGENT_RATE_LIMIT_XXX`](src/config.ts) 中配置。
...
主要是在 [System Prompt](src/chat-user/prompt.ts) 中声明安全规则，拒绝角色扮演、规则泄露等指令。
